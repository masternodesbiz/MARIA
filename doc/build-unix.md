UNIX BUILD NOTES
====================
Some notes on how to build MARIA Core in Unix.

Note
---------------------
Always use absolute paths to configure and compile MARIA Core and the dependencies,
...
This will build maria-qt as well, if the dependencies are met.
...
memory available when compiling MARIA Core. On systems with less, gcc can be
 ...
 **For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~maria/+archive/maria).
...
To build MARIA Core without wallet, see [*Disable-wallet mode*](/doc/build-unix.md#disable-wallet-mode)
...
If you want to build maria-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a maria-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip mariad" to strip the debug
...
Security
--------
To help make your MARIA Core installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, MARIA Core should be built with a non-executable stack
...
When the intention is to run only a P2P node without a wallet, MARIA Core may be compiled in
