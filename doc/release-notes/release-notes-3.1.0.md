MARIA Core version *3.1.0* is now available from:  <https://github.com/maria-project/maria/releases>
...
zMARIA Staking is here! zMARIA staking will be activated on the morning of the 8th of May 2018. With the release of zMARIA staking, there are effectively 2 versions of zMARIA, zMARIA minted on the 3.0.6 MARIA wallet or lower, and zMARIA minted on MARIA wallet version or higher. New features in this release will require the use of zMARIA v2, zMARIA minted on this wallet release 3.1.0 or later. If you currently hold zMARIA v1 and wish to take advantage of zMARIA staking and deterministic zMARIA, you will need to spend the zMARIA v1 to yourself and remint zMARIA v2.
Note: To find your zMARIA version, click the privacy tab, then the zMARIA Control button then expand the arrows next to the desired denomination.


### Deterministic zMARIA Seed Keys

zMARIA is now associated with a deterministic seed key. With this seed key, users are able to securely backup their zMARIA outside of the wallet that the zMARIA had been minted on. zMARIA can also be transferred from wallet to wallet without the need of transferring the wallet data file.


### Updated zMARIA minting

zMARIA minting now only requires 1 further mint (down from 2) to mature. zMARIA mints still require 20 confirmations.  Mints also require that the 'second' mint is at least two checkpoints deep in the chain (this was already the case, but the logic was not as precise).


### zMARIA Search

Users will now have the ability to search the blockchain for a specific serial # to see if a zMARIA denomination has been spent or not.



MARIA/zMARIA Staking and Masternode Rewards
--------------

### MARIA, zMARIA and Masternode Payment Schedule

To encourage the use of zMARIA and increase the MARIA zerocoin anonymity set, the MARIA payment schedule has been changed to the following:

If a user staking zMARIA wins the reward for their block, the following zMARIA reward will be: 
- 3 zMARIA (3 x 1 denominations) rewarded to the staker, 2 MARIA rewarded to the masternode owner and 1 MARIA available for the budget. This is a total block reward of 6 MARIA, up from 5.

If a user staking MARIA wins the reward, the following amounts will be rewarded: 
- 2 MARIA to the MARIA staker, 3 MARIA to the Masternode owner and 1 MARIA available for the budget. This is a total block reward of 6 MARIA, up from 5.


### Return change to sender when minting zMARIA

Previously, zMARIA minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the contributing address which contained the most MARIA and return the change from a zMARIA mint to that address.
...
-	Enable and disable the auto zMARIA minting feature. This is enabled by default and the enablezeromint=0 setting in the maria.conf file will overwrite the GUI option.
-	The percentage of autominted zMARIA can now be set from 1 to 100, changed from 10 – 100.
-	The stake split threshold can now be set VIA the wallet options. This setting is an advanced feature for those wishing to remain staking regular MARIA.
...
- zmariabackuppath = <directory / full path>
...
* If zMARIA backup, auto generated name is `wallet-autozmariabackup.dat-<year>-<month>-<day>-<hour>-<minute>-<second>`
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the MARIA support group, which often times sees users say "I didn't spend that zMARIA". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the MARIA masternodes, the monthly budget available to be utilised has been increased to 42,000 MARIA / month. This MARIA only has the opportunity to be raised once per month (paid to winning proposals) with any unused MARIA not created by the blockchain.

### Budget Finalization Fee

The MARIA finalization fee for successful proposals has now been reduced, this fee is now 5 MARIA down from 50 MARIA. The total fee outlay for a successful proposal is now a total of 55 MARIA.
...
-	Enforced v1 zMARIA spends to require a security level of 100
-	Updates to zMARIA spends to avoid segfaults
...
Several critical security flaws in the zerocoin protocol and MARIA's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zMARIA Control dialog in the QT wallet, a user is able to see zMARIA marked as version 1 or 2.

zPoS (zMARIA staking)
--------------

Once a zMARIA has over 200 confirmations it becomes available to stake. Staking zMARIA will consume the exact zerocoin that is staked and replace it with a freshly minted zerocoin of the same denomination as well as a reward of three 1 denomination zMARIA. So for example if a 1,000 zMARIA denomination is staked, the protocol replaces that with a fresh 1,000 denomination and three1 denomination zMARIAs.
...
Zerocoins, or zMARIA, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of zMARIA that can be recalculated at any time using the seed. Deterministic zMARIA allows for users to backup all of their future zMARIA by simply recording their seed and keeping it in a safe place (similar to backing up a private key for MARIA). The zMARIA seed needs to remain in the wallet in order to spend the zMARIA after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private zMARIA data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.
...
### Core Features
 - #582 `cb1632520` [Core] zMARIA v2: zPoS dzMARIA ezMARIA external backup and budget fixes (too many to list)
...
### GUI
 - #572 `d9b23fe60` [Qt] Refresh zMARIA balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of zMARIA status output (Mrs-X)
 ...
 - #467 `7099831bd` [Qt] Clear PrivacyDialog "zMaria Selected" labels after sending. (presstab)
...
### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on zMARIA spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting zMARIA. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending zMARIA. (presstab)
