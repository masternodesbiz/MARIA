MARIA Core version *v5.0.0* is now available from:  <https://github.com/maria-project/maria/releases>
...
#### Overview
Users will be able to protect their financial information by sending and receiving MARIAs privately, as well as sending and receiving encrypted messages attached to the shield transactions.
More visual information about the protocol can be found at https://www.mariacoin.com .
...
* Top bar showing shield MARIA balances.
...
* New derivation path for Sapling keys: Shield addresses are derived from the same wallet master seed used for the deterministic derivation of all other keys. The same seed, therefore, can be used to restore both transparent and Shield addresses, recovering both transparent and Shield MARIA balance.
...
See the release notes from [v4.2.0](https://github.com/hostmaria/mariacoin/blob/master/doc/release-notes/release-notes-4.2.0.md#label-and-account-apis-for-wallet) for a full description of the changes from the 'account' API to the 'label' API.
...
| `getsupplyinfo` | Returns detailed MARIA supply information | No |
...
* `viewshieldtransaction`
  ...
  Result:
  {
    ...
    "fee"  : x.xxx,             (numeric) The transaction fee in MARIA
    "spends" : [
      {
        ...
        "value" : x.xxx                 (numeric) The amount in MARIA
...
* `getinfo`
  The `zerocoinbalance` JSON return entry and the `zMARIAsupply` return JSON object have been removed due to zerocoin deprecation.
  The `balance` JSON return entry is now all inclusive (the sum of all transparent and shield MARIA)
  The `moneysupply` JSON return entry is the total MARIA supply (the sum of all transparent and shield MARIA)
...
| `getzerocoinbalance` | Returned the wallet's total zMARIA balance | Zerocoin deprecation |
| `listmintedzerocoins` | List all zMARIA mints in the wallet | Zerocoin deprecation |
| `listzerocoinamounts` | List information about your zerocoin amounts | Zerocoin deprecation |
| `listspentzerocoins` | List all the spent zMARIA mints in the wallet | Zerocoin deprecation |
| `mintzerocoin` | Mint the specified zMARIA amount | Zerocoin deprecation |
| `spendzerocoin` | Spend zMARIA to a MARIA address | Zerocoin deprecation |
| `spendzerocoinmints` | Spend specific zMARIA mints to a MARIA address | Zerocoin deprecation |
...
| `reconsiderzerocoins` | Check archived zMARIAs to see if any mints were added to the chain | Zerocoin deprecation |
| `setzmariaseed` | Set the wallet's deterministic zMARIA seed to a specific value | Zerocoin deprecation |
| `getzmariaseed` | Return the current deterministic zMARIA seed | Zerocoin deprecation |
| `generatemintlist` | Show mints that are derived from the deterministic zMARIA seed | Zerocoin deprecation |
| `dzmariastate` | Show the current state of the mintpool of the wallet | Zerocoin deprecation |
| `searchdzmaria` | Do a search for deterministically generated zMARIA that have not yet added to the wallet | Zerocoin deprecation |
