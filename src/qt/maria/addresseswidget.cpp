...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto mariaAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(mariaAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(mariaAdd);
        if (walletModel->updateAddressBookLabels(mariaAdd, label.toUtf8().constData(),
