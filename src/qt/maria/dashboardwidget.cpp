// Copyright (c) 2019-2020 The MARIA developers
    ...
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareMaria, "square-chart-maria");
    setCssProperty(ui->labelSquareMN, "square-chart-mn");
    setCssProperty(ui->labelMaria, "text-chart-maria");
    ...
    setCssProperty(ui->labelAmountMaria, "text-stake-maria-disable");
...
QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isMaria = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::MNReward;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair MARIA, MN Reward
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            maria = (pair.first != 0) ? pair.first / 100000000 : 0;
            mn = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalMaria += pair.first;
            chartData->totalMN += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesMaria.append(maria);
        chartData->valuesMN.append(mn);

        int max = std::max(maria, mn);
...
void DashboardWidget::onChartRefreshed()
{
	...
	QColor gridY;
    set0->setColor(QColor(41,100,30));
    set1->setColor(QColor(255,215,0));
    ...
    set0->append(chartData->valuesMaria);
    set1->append(chartData->valuesMN);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalMaria > 0 || chartData->totalMN > 0) {
        setCssProperty(ui->labelAmountMaria, "text-stake-maria");
        setCssProperty(ui->labelAmountMN, "text-stake-mn");
    } else {
        setCssProperty(ui->labelAmountMaria, "text-stake-maria-disable");
        setCssProperty(ui->labelAmountMN, "text-stake-mn-disable");
    }
    forceUpdateStyle({ui->labelAmountMaria, ui->labelAmountMN});
    ui->labelAmountMaria->setText(GUIUtil::formatBalance(chartData->totalMaria, nDisplayUnit));
    ...
    if (fShowCharts) {
        if (!stakesFilter) {
            ...
            stakesFilter->setTypeFilter(TransactionFilterProxy::TYPE(TransactionRecord::StakeMint) |
                                        ...
                                        TransactionFilterProxy::TYPE(TransactionRecord::StakeZMARIA) |
