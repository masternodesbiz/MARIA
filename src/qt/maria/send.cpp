    ...
    coinIcon(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-maria");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onMARIASelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onMARIASelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent MARIAs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res) {
                inform(tr("Error generating address to shield MARIAs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent MARIAs to shield."));
    }
}
...
void SendWidget::onMARIASelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto mariaAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(mariaAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(mariaAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(mariaAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
