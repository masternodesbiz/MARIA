    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopMaria, ui->labelAmountTopShieldedMaria}, "amount-small-topbar");
    setCssProperty({ui->labelAmountMaria}, "amount-topbar");
    setCssProperty({ui->labelPendingMaria, ui->labelImmatureMaria}, "amount-small-topbar");
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    // MARIA Total
    QString totalMaria = GUIUtil::formatBalance(newBalance.balance, nDisplayUnit);
    ...
    // MARIA
    // Top
    ui->labelAmountTopMaria->setText(totalTransparent);
    ui->labelAmountTopShieldedMaria->setText(totalShielded);
    // Expanded
    ui->labelAmountMaria->setText(totalMaria);
    ui->labelPendingMaria->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance + newBalance.unconfirmed_shielded_balance, nDisplayUnit));
    ui->labelImmatureMaria->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
}
