...
UniValue preparebudget(const JSONRPCRequest& request)
{
    ...
    if (!pwallet->CreateBudgetFeeTX(wtx, nHash, keyChange, BUDGET_FEE_TX_OLD)) { // 50 MARIA collateral for proposal
...
UniValue getbudgetprojection(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "    \"TotalPayment\": xxx.xxx,      (numeric) Total payment amount in MARIA\n"
            "    \"MonthlyPayment\": xxx.xxx,    (numeric) Monthly payment amount in MARIA\n"
            ...
            "    \"Allotted\": xxx.xxx,           (numeric) Amount of MARIA allotted in current period\n"
            "    \"TotalBudgetAllotted\": xxx.xxx (numeric) Total MARIA allotted\n"
            "  }\n"
            "  ,...\n"
            "]\n"
