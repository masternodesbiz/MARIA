...
CMariaStake* CMariaStake::NewMariaStake(const CTxIn& txin, int nHeight, uint32_t nTime)
{
    if (txin.IsZerocoinSpend()) {
        error("%s: unable to initialize CMariaStake from zerocoin spend", __func__);
        return nullptr;
    }
    ...
    if (!coin.IsSpent()) {
        ...
        // All good
        return new CMariaStake(coin.out, txin.prevout, pindexFrom);
    }
    ...
    // All good
    return new CMariaStake(txPrev->vout[txin.prevout.n], txin.prevout, pindexFrom);
}

bool CMariaStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

CTxIn CMariaStake::GetTxIn() const
{
    return CTxIn(outpointFrom.hash, outpointFrom.n);
}

CAmount CMariaStake::GetValue() const
{
    return outputFrom.nValue;
}

CDataStream CMariaStake::GetUniqueness() const
{
    //The unique identifier for a MARIA stake is the outpoint
...
const CBlockIndex* CMariaStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CMariaStake: uninitialized pindexFrom");
    return pindexFrom;
}
