...
BOOST_AUTO_TEST_CASE(TransparentToSapling)
{
    ...
    // 0.5 t-MARIA in, 0.4 shielded-MARIA out, 0.1 t-MARIA fee
...
BOOST_AUTO_TEST_CASE(SaplingToSapling)
{
    ...
    // --- 0.4 shielded-MARIA in, 0.299 shielded-MARIA out, 0.1 shielded-MARIA fee, 0.001 shielded-MARIA change (added to fee)
    ...
    // --- Now try with 1 shielded-MARIA in, 0.5 shielded-MARIA out, 0.1 shielded-MARIA fee, 0.4 shielded-MARIA change
...
BOOST_AUTO_TEST_CASE(FailsWithNegativeChange)
{
    ...
    // Generate a 0.5 MARIA note
    auto testNote = GetTestSaplingNote(pa, 59990000);

    // Fail if there is only a Sapling output
    // 0.5 shielded-MARIA out, 0.1 t-MARIA fee
    auto builder = TransactionBuilder(consensusParams);
    ...
    // 0.5 t-MARIA out, 0.1 t-MARIA fee
    ...
    // 0.5 t-MARIA out, 0.1 t-MARIA fee, 0.59999 shielded-MARIA in
    builder.AddSaplingSpend(expsk, testNote.note, testNote.tree.root(), testNote.tree.witness());
    BOOST_CHECK_EQUAL("Change cannot be negative", builder.Build().GetError());
