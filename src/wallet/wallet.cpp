...
// Copyright (c) 2015-2021 The MARIA developers
...
/**
 * Fees smaller than this (in umaria) are considered zero fee (for transaction creation)
                                      ...
                                      std::string& strError)
{
    ...
    if (txOut.nValue != consensus.nMNCollateralAmt) {
        strError = strprintf("Invalid collateral tx value, must be %s MARIA", FormatMoney(Params().GetConsensus().nMNCollateralAmt));
...
bool CWallet::CreateCoinstakeOuts(const CMariaStake& stakeInput, std::vector<CTxOut>& vout, CAmount nTotal) const
{
        ...
        bool stopOnNewBlock) const
{
    ...
    for (auto it = availableCoins->begin(); it != availableCoins->end();) {
        ...
        CMariaStake stakeInput(it->tx->tx->vout[it->i],
                             outPoint,
                             it->pindex);
