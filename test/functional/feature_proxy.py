...
"""Test mariad with different proxy configuration.

Test plan:
- Start mariad's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on mariad side:
...
- Create mariads that connect to them
- Manipulate the mariads using addnode (onetry) an observe effects
...
class ProxyTest(MariaTestFramework):
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        # Note: mariad's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
