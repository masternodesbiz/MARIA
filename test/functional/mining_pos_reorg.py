...
class ReorgStakeTest(MariaTestFramework):
    ...
    def check_money_supply(self, expected_maria):
        # verify that nodes have the expected MARIA supply
        maria_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(maria_supply, [DecimalAmt(expected_maria)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that MARIA supply was properly updated after the reorgs (including burned fee)
        self.log.info("Check MARIA supply...")
